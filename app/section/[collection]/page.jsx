...
async function getChapters(collectionName) {
...
export default async function SectionPage({ params }) {
  ...
  const chapters = await getChapters(collection);
        ...
        {chapters.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {chapters.map((chapter, index) => (
              <Link key={chapter} href={`/quiz/${collection}/${encodeURIComponent(chapter)}`} className="group">
